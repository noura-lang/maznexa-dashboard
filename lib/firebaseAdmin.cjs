// CommonJS (.cjs) deliberately — the project's package.json has
// "type": "module", so a plain .js file here would load as ESM, and
// firebase-admin's own dependency chain (jwks-rsa) does an internal
// require() that breaks under Node's ESM loader with:
//   Error [ERR_REQUIRE_ESM]: require() of ES Module .../jwks-rsa/src/index.js
// A .cjs extension always loads as CommonJS regardless of "type": "module",
// which keeps firebase-admin's whole dependency graph in one consistent
// module system and avoids that interop failure.
//
