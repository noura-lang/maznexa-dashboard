...
async function verifyEmployee(req) {
  ...
  const rawAdminValue = permissionRow?.['Admin']
  const isAdmin = parseBool(rawAdminValue)

  // Server-side only — diagnoses whether Admin is being detected correctly
  // for a given account: was a Permissions row even found for this email,
  // what's the exact raw cell value read from the "Admin" column (so a
  // sheet-data problem — wrong text, wrong column, empty cell — shows up
  // clearly rather than just as "not admin"), and the resulting boolean.
  console.log('verifyEmployee: Admin check', {
    email,
    permissionRowFound: !!permissionRow,
    rawAdminValue,
    isAdmin,
  })
